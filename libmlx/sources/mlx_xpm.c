...
/*   Updated: 2016/06/29 18:02:04 by bsouchet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#define	RETURN	{ if (colors) free(colors); if (tab) free(tab); \
	...
	if (img) mlx_destroy_img(xvar, img);   \
	return ((void *)0); }
				...
				int info_size, char *(*f)())
{
	...
	if (!(img = mlx_new_img(xvar, width, height)))
		RETURN;
	opp = 4;
	i = height;
	data = img->buffer;
...
void			*mlx_xpm_to_img(mlx_ptr_t *xvar, char *file)
{
	int				fd;
	int				size;
	char			*ptr;
